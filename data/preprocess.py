...
# import os
# from tqdm import tqdm
def trim_aria_videos(source_path, save_path, untrack_csv=None):
    """
    Trim long videos into 5-second clips and bypass the fps issue.
    Follows the same pattern as trim_ego4d_videos.

    :param source_path: Long video path
    :param save_path: Path to save the trimmed clips
    :param untrack_csv: Used to remove untracked frames (optional)
    :return: None
    """
    os.makedirs(save_path, exist_ok=True)
    
    untracked = dict()
    if untrack_csv and os.path.exists(untrack_csv):
        with open(untrack_csv, 'r') as f:
            lines = [item for item in csv.reader(f)]
        for line in lines:
            start_hr, start_min, start_sec = line[1].split(':')
    ...
    for item in tqdm(sorted(os.listdir(source_path))):
        ...
        if os.path.splitext(item)[-1] == '.mp4':
            ...
            # Handle case where fps is None
            if fps is None:
                fps = 20.0  # Set default fps value

            vid = os.path.splitext(item)[0]
            os.makedirs(os.path.join(save_path, vid), exist_ok=True)

            # Use exact 5-second clips like in ego4d function (not 2-second starts with 5-second length)
            for i in tqdm(range(0, int(duration), 5), leave=False):
                start, end = i, i + 5
                if end > duration:
                    break
                
                # Check if this segment should be skipped (if using untrack_csv)
                if untrack_csv and vid in untracked.keys():
                    skip = False
                    for interval in untracked[vid]:
                        if not (end < interval[0] or start > interval[1]):
                            skip = True
                ...
                # Using ffmpeg to precisely cut a 5-second clip
                output_file = os.path.join(save_path, vid, f'{vid}_t{start}_t{end}.mp4')
                
                ffmpeg_command = [
                    'ffmpeg', '-y',
                    '-ss', str(start),
                    '-i', os.path.join(source_path, item),
                ...
                try:
                    ...
                    print(f"Error processing clip {vid} from {start} to {end}: {e}")
                    continue
# def trim_aria_videos(source_path, save_path):
#     """
#     Trim long videos into clips and bypass the fps issue.
...
#                 # Make sure the clip has a valid fps value before writing
#                 if clip.fps is None:
#                     clip.fps = fps  # Set clip's fps explicitly

#                 print('clip fps==', clip.fps)

#                 # Using ffmpeg directly to write the video clip with the correct fps
#                 output_file = os.path.join(save_path, vid, f'{vid}_t{start}_t{end-1}.mp4')

#                 # Write the video clip using ffmpeg (subprocess)
#                 ffmpeg_command = [
#                     'ffmpeg', '-y', '-i', os.path.join(source_path, item), 
#                     '-vf', f"fps={fps}", 
#                     '-c:v', 'libx264', 
#                     '-preset', 'fast', 
#                     '-crf', '22',  # Quality level (lower is better quality)
#                     output_file
#                 ]
...
def get_aria_frame_label(data_path, save_path):
    ...
    for group in os.listdir(os.path.join(data_path, 'labels')):
        ...
        for i in tqdm(range(len(meta_rows))):
            ...
            # fps of video is 20 but it's 10 for g aze. We need to interpolate it.
...
def trim_audio(audio_path, save_path, vid, duration):
    os.makedirs(os.path.join(save_path, vid), exist_ok=True)
    
    for i in range(0, int(duration), 5):
        start, end = i, i + 5
        ffmpeg_command = [
            'ffmpeg', '-i', os.path.join(audio_path, f"{vid}.wav"),
            '-ss', str(start), '-to', str(end),
            '-acodec', 'pcm_s16le', '-ac', '1', '-ar', '24000',
            os.path.join(save_path, vid, f'{vid}_t{start}_t{end}.wav')
        ]
...
def extract_audio(data_path, save_path, dataset):
    ...
    # if dataset == 'Ego4D':
    #     for vid in os.listdir(data_path):
    #         if vid=='7d8b9b9f-7781-4357-a695-c88f7c7f7591':
    #             os.makedirs(os.path.join(save_path, vid), exist_ok=True)
    #             missing_audio_path = os.path.join('data/Ego4D/missing_audio')
    #             video_path= os.path.join('data/Ego4D/full_scale.gaze', vid+'.mp4')
    #             print(missing_audio_path)
    #             duration = get_video_duration(video_path)
    #             trim_audio(missing_audio_path, save_path, vid, duration)
        # for audio_file in os.listdir(missing_audio_path):
        #     if audio_file.endswith('.wav'):  # Ensure we're processing WAV files
        #         ffmpeg_command = ['ffmpeg', '-i', os.path.join(missing_audio_path, audio_file),
        #                         '-vn', '-acodec', 'pcm_s16le', '-ac', '1',  # Convert to mono
        #                         '-ar', '24000',  # Convert sample rate to 24kHz
        #                         os.path.join(save_path, audio_file)]
        #         subprocess.call(ffmpeg_command)
...
def main():
    ...
    # path_to_ego4d = 'data/Ego4D'  # change this to your own path
    ...
    path_to_aria = 'data/Aria_data'
    
    # source_path = f'{path_to_aria}/full_scale'
    # source_path = f'{path_to_aria}'
