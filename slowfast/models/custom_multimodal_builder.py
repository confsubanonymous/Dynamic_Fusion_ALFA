...
from slowfast.models.ALFA import ALFAPolicy

class RLPolicy_sp(nn.Module):
...
class CSTS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # =============================== RL Policy for Modality Prioritization ===============================
        self.spatial_policy = ALFAPolicy(input_dim=768 * 2, temp=2)  # Assuming embed_dim = 768
        self.temporal_policy= ALFAPolicy(input_dim=768 * 2, temp=5)
    ...
    def forward(self, x, y, return_embed=False, return_spatial_attn=False, return_temporal_attn=False):
        ...
        B, N_v, C_v = x_spatial.size()  # [4, 256, 768]
        B_a, T_a, C_a = y_spatial.size()  # [4, 4, 768]

        # Process each time frame separately for spatial fusion
        fusion_weights_spatial = []
        x_spatial_weighted_frames = []
        y_spatial_weighted_frames = []

        # We know T = 4, and each frame has 64 spatial tokens (256/4)
        spatial_tokens_per_frame = N_v // T_a  # Should be 64
        x_global = x_spatial.mean(dim=1)  # Global visual representation [4, 768]
        y_global = y_spatial.mean(dim=1) 

        for t in range(T_a):
            # Get spatial tokens for this frame
            start_idx = t * spatial_tokens_per_frame
            end_idx = (t + 1) * spatial_tokens_per_frame
            
            # Extract frame features
            x_frame = x_spatial[:, start_idx:end_idx, :]  # [4, 64, 768]
            x_frame_mean = x_frame.mean(dim=1)  # [4, 768]
            y_frame = y_spatial[:, t, :]  # [4, 768]
            
            # Get fusion weight from spatial policy
            fusion_weight_frame = self.spatial_policy(x_frame_mean, y_frame, x_global, y_global)  # [4, 1]

            fusion_weights_spatial.append(fusion_weight_frame)
            
            # Apply weights to embeddings
            x_frame_weighted = fusion_weight_frame.unsqueeze(1) * x_frame  # [4, 64, 768]
            y_frame_weighted = (1 - fusion_weight_frame).unsqueeze(1) * y_frame.unsqueeze(1)  # [4, 1, 768]
            
            # Store weighted embeddings
            x_spatial_weighted_frames.append(x_frame_weighted)
            y_spatial_weighted_frames.append(y_frame_weighted)

        # Recombine spatial embeddings
        x_spatial_weighted = torch.cat(x_spatial_weighted_frames, dim=1)  # [4, 256, 768]
        y_spatial_weighted = torch.cat(y_spatial_weighted_frames, dim=1)  # [4, 4, 768]

        # Concatenate for fusion
        av_spatial = torch.cat([x_spatial_weighted, y_spatial_weighted], dim=1)  # [4, 260, 768]

        # Store fusion weights for loss computation
        spatial_fusion_weights = torch.cat(fusion_weights_spatial, dim=0)  # [16, 1]
        ...
        fusion_weights_temporal = []
        x_temporal_weighted = []
        y_temporal_weighted = []

        x_global = x_temporal.mean(dim=1)  # Global visual representation [4, 768]
        y_global = y_temporal.mean(dim=1) 

        for t in range(x_temporal.size(1)):  # Should be 4 time steps
            # Get features for this time step
            x_t = x_temporal[:, t, :]  # [4, 768]
            y_t = y_temporal[:, t, :]  # [4, 768]
            
            # Get fusion weight
            fusion_weight_t = self.temporal_policy(x_t, y_t, x_global, y_global)  # [4, 1]
            # device = x_t.device
            # fusion_weight_t = torch.full((x_t.size(0), 1), 1.0, device=device)
            fusion_weights_temporal.append(fusion_weight_t)
            
            # Apply weights
            x_weighted_t = fusion_weight_t * x_t  # [4, 768]
            y_weighted_t = (1 - fusion_weight_t) * y_t  # [4, 768]

            
            
            # Store weighted features
            x_temporal_weighted.append(x_weighted_t.unsqueeze(1))  # [4, 1, 768]
            y_temporal_weighted.append(y_weighted_t.unsqueeze(1))  # [4, 1, 768]

        # Recombine temporal features
        x_temporal_weighted = torch.cat(x_temporal_weighted, dim=1)  # [4, 4, 768]
        y_temporal_weighted = torch.cat(y_temporal_weighted, dim=1)  # [4, 4, 768]
       
        temporal_fusion_weights = torch.cat(fusion_weights_temporal, dim=0)  # [16, 1]
        # Fuse
        av_temporal = torch.cat([x_temporal_weighted, y_temporal_weighted], dim=1)  # [B, 2T, 768]
        ...
        if not return_embed and not return_spatial_attn and not return_temporal_attn:
            ...
            return [feat, x_return, y_return, spatial_fusion_weights, temporal_fusion_weights]
