...
class Ego4d_av_gaze_forecast(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        if self.mode == 'train' or self.mode == 'val':
            path_to_file = self.cfg.DATA.TRAIN_CSV 
        elif self.mode == 'test':
            path_to_file = self.cfg.DATA.TEST_CSV
        else:
            raise ValueError(f"Don't support mode {self.mode}.")
        # print('mode', self.mode)
        # print('path', path_to_file)
        # print(self.cfg.DATA.TEST_CSV)
        ...
        # Shuffle paths if we're in 'train' mode
        # if self.mode == 'train':
        #     random.shuffle(paths)

        # Split the paths: 90% for training, 10% for validation
        train_size = int(0.9 * len(paths))

        # val_size = len(paths) - train_size

        # For training, use 90% of the data
        train_paths = paths[:train_size]

        # For validation, use the remaining 10%
        val_paths = paths[train_size:]
        # print(self.mode)
        # Now we build the paths for videos based on the mode
        if self.mode == 'train':
            paths = train_paths
        elif self.mode == 'val':
            paths = val_paths
        ...
        for video_path in self._path_to_videos:
            self._path_to_audios.append(video_path.replace('clips.gaze', 'clips.audio_24kHz_stft').replace('.mp4', '.npy'))
        ...
        for path in tqdm(self._path_to_videos):
            ...
            if video_name in self._labels.keys():
                ...
                # prefix=self.cfg.DATA.PATH_PREFIX
        ...
        # print(len(self._path_to_audios))
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            labels_global_index = np.clip(labels_global_index, 0, self._labels[video_name].shape[0] - 1)
